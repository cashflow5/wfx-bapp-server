...
public class FinanceServiceImpl extends BaseServiceImpl implements IFinanceService {
	...
	public CommissionSummary getCommissionSummary(UserInfo userInfo) {
		...
		if (sInfo != null) {
			// 未结算
			result.setNotSettled(toDouble(sInfo.getUnSettlementAmount()));
			// 已经结算
	...
	public CommissionList getCommissionList(CommissionSearcher searcher) {
		...
		PageModel<CommissionDetailOutputDto> comms1 = commissionDetailFrontApi.findPage(inputDto, page);
		if (null == comms1) {
			...
			if (items1 != null && items1.size() > 0) {
				for (CommissionDetailOutputDto item : items1) {
					...
					commission.setId(item.getId());
					commission.setAmount(toDouble(item.getCommissionAmount()));
					commission.setOrderNo(item.getWfxOrderDetailNo());
		...
		PageModel<CommissionDetailOutputDto> comms2 = commissionDetailFrontApi.findPage(inputDto, page);
		if (comms2 != null) {
	...
	public BankCardInfo bindBankCard(BankCardInfo bankCardInfo) {
		...
		String bankNo = bankCardInfo.getBankNo();
		if (isBlank(bankNo)) {
			return bankCardInfo;
		}
		Map<Integer, String> allBankList = sellerInfoFrontApi.getAllBankList();
		try {
			...
			if (!isBlank(s)) {
				// 开户行
				inputDto.setBankName(s);
				bankCardInfo.setBank(s);
			} else {
				return bankCardInfo;
			}
		} catch (Exception ex) {
	...
	public BankCardInfo changeBankCard(BankCardInfo bankCardInfo) {
		...
		// 身份证
		inputDto.setIdCardNo(bankCardInfo.getIdCode());
		if (inputDto.getIdCardNo() != null && inputDto.getIdCardNo().contains("*")) {
			inputDto.setIdCardNo("");
		}
		// 真实姓名
		inputDto.setTrueName(bankCardInfo.getRealName());
		if (inputDto.getTrueName() != null && inputDto.getTrueName().contains("*")) {
			inputDto.setTrueName("");
		}
		// 银行卡号
		inputDto.setBankAccount(bankCardInfo.getCardNo());
		if (inputDto.getBankAccount() != null && inputDto.getBankAccount().contains("*")) {
			inputDto.setBankAccount("");
		}
		// 银行编号
		inputDto.setBankNo(bankCardInfo.getBankNo());
		String bankNo = bankCardInfo.getBankNo();
		...
		if (bank != null) {
			...
			if (authorOld != null) {
				// 委托授权书扫描件
				if (isBlank(author.getAuthorizePic()))
					author.setAuthorizePic(authorOld.getAuthorizePic());
				// 身份证扫描件
	...
	public BooleanResult applyCash(CashApply apply) {
		...
		String shopId = apply.getUserInfo().getShopId();
		if (isBlank(sellerId) || isBlank(shopId)) {
	...
	@Override
	public CashSetting getCashSetting(UserInfo userInfo) {
		CashSetting result = new CashSetting();
		result.setHighestAmount(10000);
		result.setLowerstAmount(50);
		return result;
	}
